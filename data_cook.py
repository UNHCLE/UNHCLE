...
class CookData(Dataset):
    def __init__(self, data_path):
        ...
        self.loaded_data = np.load("all_video_features_200_validation.npy", allow_pickle=True)#self.load_data(self.data_feat)#
    ...
    def load_data(self, inp):
        combine_tensor = np.zeros((len(inp), 32, 512))
        downsample_frames = np.zeros((32, 512))
        fps_list = []
        print(len(inp))
        for indexes in range(len(inp)):
            # print(indexes)
    ...
    #         print(feat_path)
            ...
            ctr = 0
            if sz >= 32:
                for tmp in range(0,sz,max(1, (sz//32))):
                ...
                relevant_frames = np.vstack((relevant_frames, np.zeros((32-sz, 512))))
            relevant_tensor = torch.FloatTensor(relevant_frames)

            combine_tensor[indexes] = relevant_tensor
        
        # save_arr = np.array(combine_tensor)
        np.save("all_video_features_32_validation.npy", combine_tensor)
